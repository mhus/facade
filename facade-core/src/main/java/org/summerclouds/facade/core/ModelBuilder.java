...
import org.summerclouds.facade.core.Field.SEQUENCE;
...
public class ModelBuilder extends MLog {
	...
	public ModelBuilder(Source source) {
		this(source, new Model());
	}
	
	public ModelBuilder(Source source, Model gen) {
		this.gen = gen;
		this.source = source;
	}
	...
	private void loadDefinition(MUri uri) throws MException {
        Scheme scheme = source.getSchemes().get(uri);
        String content = null;
        String path = null;
        ...
        ConfigType type = source.getTypes().getForPath(path);
